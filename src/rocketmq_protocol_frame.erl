...
% BornTimestamp
% Properties
send_message_v2(Opaque, ProducerGroup, Topic, QueueId, Payload) ->
...
parse(<<_Len:32, HeaderLen:32, HeaderData:HeaderLen/binary, Payload/binary>>) ->
    case Payload == <<>> of
        true -> {jsonr:decode(HeaderData), undefined};
        false -> {jsonr:decode(HeaderData), jsonr:decode(Payload)}
    end.
